...
class CayleyTable(inputTable: Array[Array[Element]]){
    /** Accessor for the table */
    def getTable(): Array[Array[Element]] = inputTable

    private val elements = inputTable(0)
    
    private def isLatin(): Boolean = {
        def containsAllElements(set: Array[Element]): Boolean = {
            var hasAll = true
            for(elem <- elements){
...
*/
class Group(elements: Array[Element], cayleyTable: CayleyTable, generators: Array[Element]){
    /** Accessor for the Cayley Table */
    def getTable(): Array[Array[Element]] = cayleyTable.getTable()

    /** Accessor for the list of elements */
    def getElements(): Array[Element] = elements

    /** Accessor for the list of generators */
    def getGenerators(): Array[Element] = generators

    /** @return the element g*h */
    def multiply(g: Element, h: Element): Element = {
        val gIndex = elements.indexOf(g)
        val hIndex = elements.indexOf(h)

        return cayleyTable.getTable()(gIndex)(hIndex)
    }
}